...
def Wmn_get_data(username, checker_dict):
...
def get_Wmn(username):
    Sites = get_sites_from_github()
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = [executor.submit(Wmn_get_data, username, checker_dict) for checker_dict in Sites]

    for future in concurrent.futures.as_completed(futures):
        response = future.result()
        responses_main.append(response)
